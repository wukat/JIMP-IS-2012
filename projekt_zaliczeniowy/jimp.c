...
int main(int argc, char **argv)
{
  ...
  if (argc == 1)
  { printf("Nie podales katalogu!"); exit(1); }
...
hurtownia * usun_przeterminowane(hurtownia * hurtownie, int ilosc_hurtowni, data dzis)
{
  ...
  {
    j = 0;
    while ((roznica_dat(dzis, hurtownie[i].produkty[j].waznosc) >= 0) && ((hurtownie[i].produkty[j].waznosc.dzien != 0)
    && (hurtownie[i].produkty[j].waznosc.miesiac != 0) && (hurtownie[i].produkty[j].waznosc.rok != 0)) && (j < hurtownie[i].ilosc_pozycji))
    {
...
void stworz_liste_przeterminowanych(int prog, hurtownia * hurtownie, int ilosc_hurtowni, data dzis, char * katalog)
{
  ...
  for (i = 0; i < ilosc_hurtowni; i++)
    ...
    {
      wpisz = 1;
      if ((roznica_dat(hurtownie[i].produkty[j].waznosc, dzis) <= prog) && ((hurtownie[i].produkty[j].waznosc.dzien != 0)
      && (hurtownie[i].produkty[j].waznosc.miesiac != 0) && (hurtownie[i].produkty[j].waznosc.rok != 0)))
      {
        ...
        if (wpisz == 1)
        {
          ...
          { printf("Blad alokacji pamieci"); exit(1); }
...
void realizuj_to_zlecenie(zamowienie zlecenie, int ** odleglosci, int l_miast, hurtownia * hurtownie, int l_hurtowni, FILE * realizacja)
{
  ...
  fprintf(realizacja, "%d %d %d %d %s", zlecenie.termin.dzien, zlecenie.termin.miesiac, zlecenie.termin.rok, zlecenie.numer_zl, zlecenie.miasto);
            ...
            if (przydatne_hurtownie[i].produkty[j].ilosc > 0)
            {
              for (l = 0; l < zlecenie.ilosc_pozycji; l++)
              {
                if (zlecenie.pozycje[l].ilosc > 0)
                {
                  ...
                  {
                    if (check == 0)
                    { fputs(przydatne_hurtownie[i].miasto, realizacja); check = 1; } 
                    if (zlecenie.pozycje[l].ilosc >= przydatne_hurtownie[i].produkty[j].ilosc)
                    { 
                      fprintf(realizacja, "  %d %.2f %s", przydatne_hurtownie[i].produkty[j].ilosc, przydatne_hurtownie[i].produkty[j].cena, przydatne_hurtownie[i].produkty[j].nazwa);
                      zlecenie.pozycje[l].ilosc -= przydatne_hurtownie[i].produkty[j].ilosc;
                      cena += przydatne_hurtownie[i].produkty[j].ilosc * przydatne_hurtownie[i].produkty[j].cena;
                      przydatne_hurtownie[i].produkty[j].ilosc = 0;
                    }
                    else
                    {
                      fprintf(realizacja, "  %d %.2f %s", zlecenie.pozycje[l].ilosc, przydatne_hurtownie[i].produkty[j].cena, przydatne_hurtownie[i].produkty[j].nazwa);
...
void wpisz_hurtownie_do_pliku(hurtownia * hurtownie, int liczba_hurtowni, char * katalog)
{
  ...
  for (i = 0; i < liczba_hurtowni; i++)
  {
    ...
    for (j = 0; j < hurtownie[i].ilosc_pozycji; j++)
      fprintf(akt_hurtownie, "%ld %d %.2f %d %d %d\n", hurtownie[i].produkty[j].ID, hurtownie[i].produkty[j].ilosc, hurtownie[i].produkty[j].cena, hurtownie[i].produkty[j].waznosc.dzien, hurtownie[i].produkty[j].waznosc.miesiac, hurtownie[i].produkty[j].waznosc.rok);
  }
  fputc('\n', akt_hurtownie);
...
void szukaj_wyczerpanych(zamowienie zlecenie, int ** odleglosci, int l_miast, hurtownia * hurtownie, int l_hurtowni, FILE * koncowki, data dzis)
{
  ...
  for (i = 0; i < liczba_hurtowni; i++)
  {
    for (j = 0; j < hurtownie[i].ilosc_pozycji; j++)
    {
      ...
      if (znajdz == 0)
      {
        ...
        { printf("Blad alokacji pamieci!"); exit(1); }
        if ((spis[*dlugosc - 1].nazwa = malloc((strlen(hurtownie[i].produkty[j].nazwa) + 1) * sizeof(char))) == NULL)
        { printf("Blad alokacji pamieci!"); exit(1); }
...
hurtownia * lista_przydatnych_hurtowni(zamowienie zlecenie, hurtownia * hurtownie, int liczba_hurtowni, int * liczba_przydatnych_hurtowni)
{
  ...
  for (i = 0; i < liczba_hurtowni; i++)
  {
    ...
    for (j = 0; j < hurtownie[i].ilosc_pozycji; j++)
    {
      if (znalezione == 0) 
      {
        ...
        {
          if ((zlecenie.pozycje[k].ilosc != 0) && (hurtownie[i].produkty[j].ilosc != 0) && (zlecenie.pozycje[k].ID == hurtownie[i].produkty[j].ID))
          {
            znalezione = 1;
            licznik++;
          }
