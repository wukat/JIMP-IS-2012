...
hurtownia * lista_hurtowni(artykul * artykuly, int l_artykulow, int * l_hurtowni, char ** miasta, int l_miast, char * katalog)
{
  FILE * hurtownie;
  char nazwa[80];
  long int ID;
  ...
  for (i = 0; i < liczba_hurtowni; i++)
  {
    ...
    while (zamiana)
    {
      ...
      for (j = 0; j < hurtownie[i].ilosc_pozycji - k; j++)
      {
        ...
        (hurtownie[i].produkty[j].waznosc.rok == 0))) && !((hurtownie[i].produkty[j + 1].waznosc.dzien == 0) &&
        (hurtownie[i].produkty[j + 1].waznosc.miesiac == 0) && (hurtownie[i].produkty[j + 1].waznosc.rok == 0)))
        {
...
zamowienie * lista_zamowien(char ** miasta, int liczba_miast, artykul * artykuly, int liczba_artykulow, int * liczba_zamowien, char * katalog)
{
  FILE * zamowienia;
  char nazwa[80];
  int numer, i, j;
