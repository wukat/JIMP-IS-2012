...
int main(void) {
   struct okreg okregi[] = { { "pierwszy", 1, 0 }, { "drugi", 2, 0 }, { "trzeci", 3, 0 }, { "czwarty", 4, 0 }, {
         "piaty", 5, 0 } };

   struct kandydat kandydaci[] = { { "Kowalski", "Jan", NULL, 35412, 1 }, { "Nowak", "Wojciech", NULL, 53, 1 }, {
         "Hopek", "Konrad", NULL, 12, 3 }, { "Kononowicz", "Marian", NULL, 542354, 4 },
         { "Kamilak", "Ola", NULL, 312, 5 }, { "Baran", "Wiktor", NULL, 12354, 3 }, { "ja", "ja", NULL, 11, 5 } };
   int *indexes, i;
...
void sortuj_glosy(int * indexes, struct kandydat * kandyd, int size) {
   ...
   while(zamiana) {
      ...
      while(i < size - j) {
         if(((kandyd + *(indexes + i))->o == (kandyd + *(indexes + i + 1))->o)
               && ((kandyd + *(indexes + i))->glosy < (kandyd + *(indexes + i + 1))->glosy)) {
            temp = *(indexes + i);
            *(indexes + i) = *(indexes + i + 1);
            *(indexes + i + 1) = temp;
            zamiana = 1;
